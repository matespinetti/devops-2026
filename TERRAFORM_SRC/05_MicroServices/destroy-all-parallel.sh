...
RUN_LOG_DIR="${LOG_ROOT}/destroy-$(date +%Y%m%d-%H%M%S)"
...
run_service_destroy() {
  ...
  echo "[${service}] terraform destroy..."
  if [[ -f "${service_dir}/terraform.tfvars" ]]; then
    terraform -chdir="${service_dir}" destroy -input=false -auto-approve -var-file=terraform.tfvars
  else
    terraform -chdir="${service_dir}" destroy -input=false -auto-approve
  fi

  echo "[${service}] completed successfully."
}

echo "Starting parallel Terraform destroy for microservices..."
...
if [[ "${#FAILED_SERVICES[@]}" -gt 0 ]]; then
  echo "Parallel destroy finished with failures in: ${FAILED_SERVICES[*]}"
  echo
...
echo "Parallel destroy completed successfully for all services."
